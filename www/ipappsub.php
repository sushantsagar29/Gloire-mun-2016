...
$postdata['position'] = 	filter_var($postdata['position'],FILTER_SANITIZE_STRING);
...
$query = "INSERT INTO ip( name,email,contact,altcontact,gender,address,dob,college,course,syear,muncount,experience,accom,
								preference,position,agency,payment,socials,feedback) 
			VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$excute=$sth->execute(array($postdata['name'],$postdata['email'],$postdata['contact'],$postdata['altcontact'],$postdata['gender'], $postdata['address'], $postdata['dob'],
							...
							$postdata['preference'],$postdata['position'],$postdata['agency'],$postdata['payment'],$postdata['socials'],$postdata['feedback']
							));
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="primary">
    <div class="light-wrapper box">
      <div class="container">
		...
		<div class="span8">
				<!-- span of the body -->
				<center><h2>International Press Corps Application Submission</h2></center>
				<?php
				if(!$excute ){
	...
	$to = $postdata['email'] ;
    $subject = "International Press Corps application for Gloire MUN 2016";
