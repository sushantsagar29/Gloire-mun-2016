...
$postdata['preference'] = 	filter_var($postdata['preference'],FILTER_SANITIZE_STRING);
$postdata['country1'] = 	filter_var($postdata['country1'],FILTER_SANITIZE_STRING);
$postdata['country2'] = 	filter_var($postdata['country2'],FILTER_SANITIZE_STRING);

$postdata['payment'] = 	filter_var($postdata['payment'],FILTER_SANITIZE_STRING);
$postdata['socials'] = 	filter_var($postdata['socials'],FILTER_SANITIZE_STRING);
...
$query = "INSERT INTO delegate( name,email,contact,altcontact,gender,address,dob,college,course,syear,muncount,experience,accom,
								preference,country1,country2,payment,socials,feedback) 
			VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$excute=$sth->execute(array($postdata['name'],$postdata['email'],$postdata['contact'],$postdata['altcontact'],$postdata['gender'], $postdata['address'], $postdata['dob'],
							$postdata['college'],$postdata['course'], $postdata['syear'], $postdata['muncount'],$postdata['experience'],$postdata['accom'],
							$postdata['preference'],$postdata['country1'],$postdata['country2'],$postdata['payment'],$postdata['socials'],$postdata['feedback']
							));
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="primary">
    <div class="light-wrapper box">
      <div class="container">
		...
		<div class="span8">
				<!-- span of the body -->
				<center><h2>DELEGATE APPLICATIONS SUBMISSION</h2></center>
				<?php
				if(!$excute ){
					echo "Error in connection"."<br/>"."Please try submitting the form again"."<br />";
					exit;
				}
				echo '<center><p text-align="center"><b>Thank you '.$postdata['name'].' for registering for Gloire MUN 2016.
				<br/>Your email will be use as reference ID. Please check you inbox for confirmation mail.
	...
	$email_message .= "Thank you for registering for Gloire MUN 2016. Your email will be use as reference ID.\n\n";
	...
	$to = $postdata['email'] ;
    $subject = "Delegate registration for Gloire MUN 2016";
