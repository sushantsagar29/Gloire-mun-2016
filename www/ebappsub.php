...
$postdata['expdel'] = 	filter_var($postdata['expdel'],FILTER_SANITIZE_STRING);
$postdata['expeb'] = 	filter_var($postdata['expeb'],FILTER_SANITIZE_STRING);
...
//insert details of the new eb table
$query = "INSERT INTO eb( name,email,contact,gender,address,dob,college,muncount,expdel,expeb,accom,
								preference1,preference2,que1,que2,que3,que4,demotion,feedback) 
			VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				
$sth = $dbh->prepare($query);
$excute=$sth->execute(array($postdata['name'],$postdata['email'],$postdata['contact'],$postdata['gender'], $postdata['address'], $postdata['dob'],
							$postdata['college'], $postdata['muncount'],$postdata['expdel'],$postdata['expeb'],$postdata['accom'],
							$postdata['preference1'],$postdata['preference2'],$postdata['que1'],$postdata['que2'],$postdata['que3'],
							$postdata['que4'],$postdata['demotion'],$postdata['feedback']
							));
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="primary">
    <div class="light-wrapper box">
      <div class="container">
		...
		<div class="span8">
				<!-- span of the body -->
				<center><h2>Executive Board Applications Submission</h2></center>
				<?php
				if(!$excute ){
					echo "Error in connection"."<br/>"."Please try submitting the form again"."<br />";
					exit;
				}
				echo '<center><p text-align="center"><b>Thank you '.$postdata['name'].' for applying as Executive Board member for Gloire MUN 2016.
	...
	$email_message .= "This is to confirm that we have received your application form for Executive Board member.\n";
	$email_message .= "Thank you for applying for Gloire MUN 2016. We will get in touch with you very soon.\n\n";
	...
	$to = $postdata['email'] ;
    $subject = "Executive Board application for Gloire MUN 2016";
